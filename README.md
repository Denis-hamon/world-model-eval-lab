...
- **v0.11**: the multi-seed extension promised in v0.9 and the paper's Section 5.5. `experiments/dmc_acrobot/cpg_sweep.py` pools three seeds at 50 episodes per arm per seed (n = 150 pooled) and sweeps the MLP's training-set size across `{200, 2 000, 20 000}`. The verdict hardens from `INCONCLUSIVE` to `MODEL BOTTLENECK` with an AC 95% CI of `[+0.191, +0.335]` *in every cell*; validation MSE drops by ~150x across the sweep while learned-arm success stays at zero. Paper updated with Section 5.6 ("capacity vs. coverage") and a refreshed abstract and conclusion. The site landing and the CPG page surface the same multi-seed table.
- **v0.12** (current): **first published-world-model adapter shipped** (TD-MPC2). `src/wmel/adapters/tdmpc2_adapter.py` pairs TD-MPC2's encoder + latent dynamics with a small post-hoc obs decoder to expose a `(state, action) -> next_state` callable compatible with `TabularWorldModelPlanner`. `experiments/dmc_acrobot/tdmpc2_cpg.py` trains TD-MPC2 on DMC Acrobot-swingup (2M env steps, model_size=1, 1-step frame-skip aligned with the wmel oracle) and runs the CPG protocol. A companion experiment `experiments/dmc_acrobot/coverage_mlp_on_tdmpc2.py` retrains v0.11's bespoke MLP on TD-MPC2 eval-mode rollouts to test the v0.11 coverage diagnosis directly. *Result at n = 10, seed 0*: oracle 0.30, TD-MPC2 dynamics 0.00, MLP-on-TD-MPC2-data 0.00, CPG = +0.300 with AC CI `[-0.06, +0.56]` and verdict `INCONCLUSIVE` in all three arms - *despite* the coverage axis moving (random 0% upright → TD-MPC2 10.6%) and MLP val MSE dropping 5x (4e-4 → 7.7e-5). The framework correctly refuses to convict "coverage alone" as the diagnosis; the paper's §4-5 conclusion needs the refinement described under "What's next" below.

## What's next

Three actions, ordered by cost / payoff:

1. **Stronger planner on the three v0.12 arms** (a few GPU-hours). Keep the three dynamics callables; replace the random-shooting MPC with CEM (or gradient-based MPC) and rerun CPG on each. The three arms currently return identical numbers; that confound is what separates *dynamics quality* from *planner capacity*. If oracle+CEM jumps and learned+CEM follows, planner is a contributor; if oracle+CEM stays at 0.30, planner is not the bottleneck.
2. **Paper v0.12** (1-2 days writing). Update §4-5 with the phase-5d refinement: the coverage thesis was tested directly, val MSE and coverage axis moved as predicted, planning success did not, framework correctly stayed `INCONCLUSIVE`. Stronger methodological argument than v0.11 (CPG separating prediction quality from decision quality, *on a result where the prediction-quality story would have over-claimed*).
3. **Multi-seed pooled-150 on the three v0.12 arms** (~50-90 GPU-hours). Reproduce seeds 1 and 2, pool to n = 150 per arm following the v0.11 sweep protocol. Likely outcome: CI compresses from `[-0.06, +0.56]` to roughly `[+0.19, +0.34]`, the same as v0.11 - which would harden the verdict to `MODEL BOTTLENECK` across all three arms simultaneously, completing the symmetry. Worth doing only after (1) has separated the planner question.
