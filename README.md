...
- **v0.14** (current): **in-episode perturbation robustness**. `experiments/dmc_acrobot/perturbation_cpg.py` sweeps `DropNextActions(k)` at $k \in \{0, 1, 5\}$ on the same three CEM arms. The `MODEL BOTTLENECK` verdict survives every cell: the oracle loses ~6 pp at $k=5$, both learned arms stay at `0/50`. The experiment is structurally one-sided (a genuine fragility test needs a regime where both arms have non-zero unperturbed success, or an observation-noise perturbation that hurts the learned arms differentially). Paper Section 5.9. Abstract and conclusion updated to reflect both new sections. HTML paper at `/paper.html` and the CPG page mirror the new tables.

## What's next

Two actions in flight or queued:

1. **Cross-environment** (in flight on `phase-5m-cartpole`). Port the four-arm CPG pipeline to DMC Cartpole-swingup. Headline so far: `MODEL BOTTLENECK` verdict reproduces at both capacities tested. Once the final size-1 pooled-30 cell lands, v0.15 ships the cross-env confirmation.
2. **Horizon-of-planning ablation under CEM** (queued, not started). Sweep $H \in \{1, 5, 10, 15, 20, 30\}$ on the existing checkpoints under CEM. Would formally attribute the dynamics-quality bottleneck to compounding error vs. distribution mismatch. Estimated 6-12 GPU-hours.
