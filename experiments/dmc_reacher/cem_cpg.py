...
Usage
-----
    python -m experiments.dmc_reacher.cem_cpg --smoke
    python -m experiments.dmc_reacher.cem_cpg                  # uses trained TD-MPC2 at results/dmc_reacher/tdmpc2_*.pt
    python -m experiments.dmc_reacher.cem_cpg --seed 1

Writes:
    results/dmc_reacher/cem_cpg.json
"""
...
from experiments.dmc_reacher.coverage_mlp_on_tdmpc2 import (
...
TDMPC2_DYNAMICS_PATH = _REPO_ROOT / "results" / "dmc_reacher" / "tdmpc2_reacher.pt"
JSON_PATH = _REPO_ROOT / "results" / "dmc_reacher" / "cem_cpg.json"
...
def _run_arm(
    *,
    ...
    env_factory = (
        eval_varied_factory(DMCReacherEnv, seed)
        if varied_init
...
def main() -> None:
    ...
    TDMPC2_DYNAMICS_PATH = _REPO_ROOT / "results" / "dmc_reacher" / f"tdmpc2_reacher{suffix}.pt"
    JSON_PATH = _REPO_ROOT / "results" / "dmc_reacher" / f"cem_cpg{suffix}.json"
    ...
    if use_tdmpc2:
        if not TDMPC2_AGENT_PATH.exists():
            raise FileNotFoundError(
                ...
                "Run experiments.dmc_reacher.tdmpc2_cpg first or pass --mlp-data-source random."
            )
    ...
    print(f"[coverage] n={coverage['n_transitions']}  dist_min={coverage['dist_min']:.3f}  "
          f"frac(d<0.05)={coverage['fraction_d_lt_0_05']:.3f}")
    ...
    def make_oracle():
        return make_reacher_oracle_dynamics()
    ...
    oracle_results, oracle_card = _run_arm(
        name="oracle dynamics", dynamics_factory=make_oracle, cfg=cfg, seed=seed, varied_init=args.varied_init,
    )
    ...
    mlp_results, mlp_card = _run_arm(
        ...
        dynamics_factory=make_mlp, cfg=cfg, seed=seed, varied_init=args.varied_init,
    )

    if tdmpc2_available:
        print("\n[3/3] CEM with TD-MPC2 dynamics...")
        tdmpc2_results, tdmpc2_card = _run_arm(
            name="TD-MPC2 dynamics", dynamics_factory=make_tdmpc2, cfg=cfg, seed=seed, varied_init=args.varied_init,
        )
    else:
