...
def main() -> None:
    ...
    oracle_results, oracle_card = _run_arm(
        name="oracle dynamics", dynamics_factory=make_oracle, cfg=cfg, seed=seed,
    )
    ...
    mlp_results, mlp_card = _run_arm(
        ...
        dynamics_factory=make_mlp, cfg=cfg, seed=seed,
    )

    if tdmpc2_available:
        print("\n[3/3] CEM with TD-MPC2 dynamics...")
        tdmpc2_results, tdmpc2_card = _run_arm(
            name="TD-MPC2 dynamics", dynamics_factory=make_tdmpc2, cfg=cfg, seed=seed,
        )
    else:
