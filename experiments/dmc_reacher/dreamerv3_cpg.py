"""Train DreamerV3 on DMC Reacher-easy, then run the CPG protocol.

Task 7 (cross-env) counterpart of `experiments.dmc_acrobot.dreamerv3_cpg`,
the third environment and the first 2-D-action env for the DreamerV3 arm.
Completes `results/MODEL_TABLE.md` as a 2-model x 3-env matrix. The CPG arm
structure is preserved verbatim from `experiments.dmc_reacher.tdmpc2_cpg`:
same `BenchmarkRunner`, same `TabularWorldModelPlanner`, same
`reacher_reach_score`, same `make_reacher_oracle_dynamics`. Only the learned
dynamics callable changes.
...
(per experiments/GPU_ROADMAP.md, Task 7):
    ...
    ARCH["obs_dim"]    = 6   (position 2 + to_target 2 + velocity 2, sorted keys)
    ARCH["action_dim"] = 2
    9 discrete action pairs persisted under the `action_set` checkpoint key
    (DMCReacherEnv.action_space), score/env from wmel.envs.dmc_reacher
"""
...
_RESULTS_DIR = _REPO_ROOT / "results" / "dmc_reacher"
...
ARCH = {
    "obs_dim": 6,        # reacher: position (2) + to_target (2) + velocity (2)
    "action_dim": 2,
    "stoch": 32,
    "discrete": 32,
    "deter": 512,
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Training workdir override (default: results/dmc_reacher/dreamerv3_workdir_seed<seed>).",
    )
    p.add_argument(
        "--out-suffix",
        default="",
...
def _port_agent_checkpoint(
    ...
    # Reacher action is 2-D: persist the explicit 9-pair discrete set (the
    # adapter rebuilds the same set via the `action_set` key), mirroring
    # experiments/dmc_reacher/tdmpc2_cpg.py.
    action_set = [list(a) for a in DMCReacherEnv().action_space]
...
def main() -> None:
    ...
    checkpoint_path = _RESULTS_DIR / f"dreamerv3_reacher{args.out_suffix}.pt"
