...
Usage
-----
    python -m experiments.dmc_acrobot.cem_cpg --smoke
    python -m experiments.dmc_acrobot.cem_cpg                  # uses trained TD-MPC2 at results/dmc_acrobot/tdmpc2_*.pt
    python -m experiments.dmc_acrobot.cem_cpg --seed 1

Writes:
    results/dmc_acrobot/cem_cpg.json
"""
...
JSON_PATH = _REPO_ROOT / "results" / "dmc_acrobot" / "cem_cpg.json"
...
def main() -> None:
    ...
    if use_tdmpc2:
        if not TDMPC2_AGENT_PATH.exists():
            raise FileNotFoundError(
                ...
                "Run experiments.dmc_acrobot.tdmpc2_cpg first or pass --mlp-data-source random."
            )
    ...
    def make_oracle():
        return make_acrobot_oracle_dynamics()
