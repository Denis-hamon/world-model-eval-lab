...
-----
    ...
    python -m experiments.dmc_acrobot.coverage_mlp_on_tdmpc2 --smoke           # 2 eps, validates wiring (no TD-MPC2 ckpt needed)
    python -m experiments.dmc_acrobot.coverage_mlp_on_tdmpc2                   # 20k transitions from results/dmc_acrobot/tdmpc2_agent.pt
    python -m experiments.dmc_acrobot.coverage_mlp_on_tdmpc2 --data-source random  # control: rerun v0.11's setup on this code path

Writes:
    results/dmc_acrobot/coverage_mlp_on_tdmpc2_cpg.json
"""
...
AGENT_PATH = _REPO_ROOT / "results" / "dmc_acrobot" / "tdmpc2_agent.pt"
JSON_PATH = _REPO_ROOT / "results" / "dmc_acrobot" / "coverage_mlp_on_tdmpc2_cpg.json"
...
def _collect_tdmpc2_rollouts(agent, env_factory, n_transitions: int, levels: tuple[float, ...], seed: int) -> list:
    """Run eval-mode TD-MPC2 episodes against wmel's DMCAcrobotEnv, snapping
...
def main() -> None:
    ...
    if use_tdmpc2:
        ...
        if not ckpt.exists():
            raise FileNotFoundError(f"TD-MPC2 agent checkpoint not found at {ckpt}. Run experiments.dmc_acrobot.tdmpc2_cpg first or pass --data-source random.")
