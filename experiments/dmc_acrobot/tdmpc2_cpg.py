"""Train TD-MPC2 on DMC Acrobot-swingup, then run the CPG protocol.
...
The CPG arm structure is preserved verbatim from `experiments.dmc_acrobot.cpg`:
same `BenchmarkRunner`, same `TabularWorldModelPlanner`, same
`acrobot_upright_score`, same `make_acrobot_oracle_dynamics`. Only the
learned dynamics callable changes.

Pipeline
--------
1. Train TD-MPC2 on DMC Acrobot-swingup for `--steps` env steps. The
...
Usage
-----
    python -m experiments.dmc_acrobot.tdmpc2_cpg --smoke         # ~5 min
    python -m experiments.dmc_acrobot.tdmpc2_cpg                 # 500k steps, ~12-20h
    python -m experiments.dmc_acrobot.tdmpc2_cpg --steps 100000  # custom
...
def _build_cfg(task: str, steps: int, seed: int) -> object:
    ...
    cfg.work_dir = str(_REPO_ROOT / "results" / "dmc_acrobot" / "tdmpc2_workdir")
...
def main() -> None:
    ...
    tdmpc2_cfg = _build_cfg(task="acrobot-swingup", steps=cfg_dict["training_steps"], seed=seed)
    set_seed(seed)
