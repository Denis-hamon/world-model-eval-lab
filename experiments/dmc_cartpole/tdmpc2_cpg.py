"""Train TD-MPC2 on DMC Cartpole-swingup, then run the CPG protocol.

This experiment is the first published-model arm of the framework. It
extends the v0.11 worked example (which paired the random-shooting planner
with a bespoke MLP and got success_rate=0 from random-policy training data)
by replacing the learned dynamics with a TD-MPC2 latent world model.

The CPG arm structure is preserved verbatim from `experiments.dmc_cartpole.cpg`:
same `BenchmarkRunner`, same `TabularWorldModelPlanner`, same
`cartpole_upright_score`, same `make_cartpole_oracle_dynamics`. Only the
learned dynamics callable changes.

Pipeline
--------
1. Train TD-MPC2 on DMC Cartpole-swingup for `--steps` env steps. The
...
Usage
-----
    python -m experiments.dmc_cartpole.tdmpc2_cpg --smoke         # ~5 min
    python -m experiments.dmc_cartpole.tdmpc2_cpg                 # 500k steps, ~12-20h
    python -m experiments.dmc_cartpole.tdmpc2_cpg --steps 100000  # custom
...
from wmel.envs.dmc_cartpole import DMCCartpoleEnv, cartpole_upright_score, make_cartpole_oracle_dynamics
...
CHECKPOINT_PATH = _REPO_ROOT / "results" / "dmc_cartpole" / "tdmpc2_cartpole.pt"
JSON_PATH = _REPO_ROOT / "results" / "dmc_cartpole" / "tdmpc2_cpg.json"
...
def main() -> None:
    ...
    oracle_results = BenchmarkRunner(
        env_factory=lambda: DMCCartpoleEnv(discrete_levels=action_levels),
