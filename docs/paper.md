...
2. **Heterogeneous verdicts across three tasks and two published models.** Exercised on three DeepMind Control Suite tasks (Acrobot-swingup, Cartpole-swingup, Reacher-easy), the gate fires four of its five branches on real data (§4.4–§4.7): `PLANNER BOTTLENECK` on Acrobot (the oracle planner itself solves only $\sim\!3\%$ of random initial states), `MODEL BOTTLENECK` on Reacher, `LEARNED OUTPERFORMS ORACLE` on high-capacity Cartpole under CEM, and `INCONCLUSIVE` on several moderate-$n$ near-ties. The same structure reproduces across two published model families (TD-MPC2 and the recurrent DreamerV3), and where both clear zero CPG ranks them. A fixed metric, or a point-estimate leaderboard, reports none of this structure.
...
### 4.7 A second published model: DreamerV3 across three environments {#sec-dreamerv3}

The cells so far use a bespoke MLP and TD-MPC2 [Hansen et al., 2024]. To test whether the verdict structure is a property of the metric rather than of one model, we add a second *published* world model, and the first *recurrent* one through the Markovian planner contract: DreamerV3 [Hafner et al., 2023]. The world-model weights are ported into the adapter (`_wm.*` keys), with numerical equivalence to the reference implementation asserted by a unit test; only the `dynamics` callable changes, so the comparison stays a clean counterfactual. The planner and ablation arms below *re-plan a single trained checkpoint* — no retraining.

<table class="paper-table">
  <thead>
    <tr><th>Environment</th><th>Planner</th><th>Oracle / Learned</th><th>Raw CPG</th><th>AC 95% CI</th><th>Verdict</th></tr>
  </thead>
  <tbody>
    <tr><td>Acrobot ($n=30$)</td><td>RS</td><td>0.033 / 0.033</td><td>+0.000</td><td>[-0.12, +0.12]</td><td><span class="verdict-pill verdict-planner-bottleneck">PLANNER BOTTLENECK</span></td></tr>
    <tr><td>Acrobot ($n=10$)</td><td>CEM</td><td>0.200 / 0.100</td><td>+0.100</td><td>[-0.24, +0.41]</td><td><span class="verdict-pill verdict-inconclusive">INCONCLUSIVE</span></td></tr>
    <tr><td>Cartpole ($n=10$)</td><td>RS</td><td>1.000 / 0.000</td><td>+1.000</td><td>[+0.61, +1.05]</td><td><span class="verdict-pill verdict-model-bottleneck">MODEL BOTTLENECK</span></td></tr>
    <tr><td>Reacher ($n=30$)</td><td>RS</td><td>1.000 / 0.667</td><td>+0.333</td><td>[+0.14, +0.49]</td><td><span class="verdict-pill verdict-model-bottleneck">MODEL BOTTLENECK</span></td></tr>
  </tbody>
</table>

DreamerV3 through CPG, task-level (varied initial state, three seeds pooled where $n=30$). Numbers from [`results/dmc_*/dreamerv3_*.json`](https://github.com/Denis-hamon/world-model-eval-lab/tree/main/results).

Three findings. First, **the verdict structure is model-independent**: DreamerV3 lands at the same three branches as TD-MPC2 — `PLANNER BOTTLENECK` on Acrobot (the search, not the model, is the limit; both arms solve $\sim\!3\%$ of random starts) and `MODEL BOTTLENECK` on Cartpole and Reacher. The verdict tracks the (environment, planner) regime, not the identity of the learned model.

Second, **where both models clear zero, CPG ranks them — and the recurrent flagship is not the better planner here**. On Reacher, DreamerV3 forfeits more planning success than TD-MPC2 ($\mathrm{CPG} = +0.333$ vs $+0.200$; §4.6); on Cartpole its learned dynamics fails outright ($\mathrm{CPG} = +1.000$, learned success $0.000$) where TD-MPC2 reaches $0.30$–$0.77$ depending on capacity (§4.5). A reconstruction- or FID-based leaderboard could rank a recurrent DreamerV3 above an MPC-tuned TD-MPC2; for *decisions* on these tasks the ranking inverts. This is the prediction-versus-decision dissociation the metric exists to surface, now between two published models rather than between a metric and a verdict.

Third, **a recurrence-truncation ablation isolates a cost the adapter pays**. The adapter's `dynamics` callable re-encodes from the observation and truncates DreamerV3's recurrent state to a one-frame posterior on every call; DreamerV3's native mode instead imagines forward in latent space from a single encode. Running both as planning arms next to the oracle, the measured CPG difference on Acrobot is $0.0$ — but *both arms sit at the planner floor* ($\sim\!10\%$ success), so this is a null with near-zero power on *decision quality*, not evidence the truncation is free. What is measurable there is *compute*: the latent rollout plans $3.4\times$ faster ($943$ vs $3{,}226$ ms per decision) by encoding once rather than per call. Quantifying the truncation's accuracy cost requires a non-floored task and is left to future work.

### 4.8 Power analysis: how many episodes before a ranking is trustworthy {#sec-power}
...
The empirical study spans three DMC control tasks (Acrobot-swingup, Cartpole-swingup, Reacher-easy), two planners (random-shooting and CEM), and three learned-dynamics families (a bespoke MLP, TD-MPC2, and the recurrent DreamerV3), each evaluated over the task's initial-state distribution with the two arms paired by initial state and three seeds pooled. The headline is not a single number but a *pattern*: across the three tasks the gate commits to four of its five branches — `PLANNER BOTTLENECK` (Acrobot), `MODEL BOTTLENECK` (Reacher, most Cartpole cells), `LEARNED OUTPERFORMS ORACLE` (high-capacity Cartpole under CEM), and `INCONCLUSIVE` (several moderate-$n$ near-ties) — and the pattern reproduces across both published model families, with CPG ranking the two where their intervals clear zero (§4.7). The study is deliberately scoped to simulated, fully-observed control where an oracle dynamics is available; on hardware-in-the-loop or physical environments the metric is undefined, and surrogate-oracle variants (e.g. a higher-fidelity model or a domain-randomised reference in the spirit of Tobin et al., 2017) are future work. The adapter interface (§2) is what makes this breadth cheap: each new cell is one more `dynamics` callable or planner constructor passed into the same `BenchmarkRunner`.
...
**Positioning.** The table below places CPG against neighbouring families by the *output* each standardly reports. CPG is not a new quantity — it is the model-exploitation gap of [Yu et al., 2020; Kidambi et al., 2020], and we claim no novelty for the subtraction. Its contribution is the *combination* in the last row: closed-loop, reported with a boundary-robust interval, and read through a verdict gate that returns `INCONCLUSIVE` rather than commit when the interval crosses zero. The value-equivalence line is complementary but distinct in kind — it reshapes *training*, whereas CPG measures a *trained* model a posteriori (§3.6).

<table class="paper-table">
  <thead>
    <tr><th>Evaluation output</th><th>Closed-loop</th><th>Calibrated CI</th><th>Gated verdict</th><th>A-posteriori</th></tr>
  </thead>
  <tbody>
    <tr><td>Reconstruction / FID / held-out loss</td><td>no</td><td>no</td><td>no</td><td>yes</td></tr>
    <tr><td>Point-estimate success leaderboard</td><td>yes</td><td>no</td><td>no</td><td>yes</td></tr>
    <tr><td>Model-exploitation gap [Yu 2020; Kidambi 2020]</td><td>yes</td><td>no</td><td>no</td><td>yes</td></tr>
    <tr><td>Value-equivalence / VAML [Grimm 2020, 2021; Farahmand 2017]</td><td>n/a</td><td>n/a</td><td>n/a</td><td>no</td></tr>
    <tr><td>Aggregate RL intervals [Agarwal 2021]</td><td>yes</td><td>yes</td><td>no</td><td>yes</td></tr>
    <tr><td><strong>CPG (this work)</strong></td><td>yes</td><td>yes</td><td>yes</td><td>yes</td></tr>
  </tbody>
</table>

Each column is the output an approach *standardly reports*: *Closed-loop* = measures planning success, not prediction fidelity; *Calibrated CI* = an interval that behaves at the boundary proportions $p \in \{0,1\}$ — a "no" means no such boundary-calibrated interval on the gap is standardly reported, even by methods that report seed-to-seed dispersion; *Gated verdict* = returns `INCONCLUSIVE` rather than commit when the interval crosses zero; *A-posteriori* = evaluates a trained model rather than reshaping training (value-equivalence / VAML are training losses, hence "n/a" for the reporting and closed-loop columns). CPG's contribution is the combination, not any single column.
