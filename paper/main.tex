...
\begin{table}[t]
  ...
  \begin{tabular}{llrrl}
    \toprule
    Learned dynamics                          & Planner       & Oracle & Learned & CPG (AC 95\% CI), verdict \\
    \midrule
    v0.11 MLP, random rollouts (20 k)         & random-shoot  & $0.30$ & $0.00$ & $+0.300\ [-0.06, +0.56]$, \textsc{inconclusive} \\
    TD-MPC2 encoder+dyn+decoder (2 M steps)   & random-shoot  & $0.30$ & $0.00$ & $+0.300\ [-0.06, +0.56]$, \textsc{inconclusive} \\
    v0.11 MLP, TD-MPC2 rollouts (20 k)        & random-shoot  & $0.30$ & $0.00$ & $+0.300\ [-0.06, +0.56]$, \textsc{inconclusive} \\
    TD-MPC2 encoder+dyn+decoder               & CEM           & $0.90$ & $0.00$ & $+0.900\ [+0.49, +1.01]$, \textsc{model bottleneck} \\
    v0.11 MLP, TD-MPC2 rollouts               & CEM           & $0.90$ & $0.00$ & $+0.900\ [+0.49, +1.01]$, \textsc{model bottleneck} \\
    \bottomrule
  \end{tabular}
  \caption{Robustness of the v0.11 reading to (a) a published-world-model adapter, (b) a different learned-dynamics data-collection policy, and (c) a stronger planner. $n = 10$ per arm, seed $0$, all other quantities identical to \S\ref{sec:empirical}. The first three rows yield identical CPG numbers under random-shooting MPC. Under CEM the oracle's success rate triples ($0.30 \to 0.90$); both learned arms remain at zero; the gap opens to $+0.900$ with a CI that no longer crosses zero. Random-shooting MPC budget: $50 \times 15 = 750$ dynamics evaluations per plan call; CEM budget: $3 \times 24 \times 15 = 1{,}080$. Numbers from \texttt{results/dmc\_acrobot/tdmpc2\_cpg.json}, \texttt{coverage\_mlp\_on\_tdmpc2\_cpg.json}, and \texttt{cem\_cpg.json}.}
  \label{tab:robustness}
\end{table}

Two readings the multi-seed sweep alone could not commit to are now decided. First, the v0.11 reading of \textsc{model bottleneck} is robust to swapping the learned dynamics for a published world model and is robust to a planner upgrade: both learned arms stay at $0/10$ under both planners. Second, the random-shooting MPC was bracketing the oracle artificially low: a CEM planner with comparable compute lifts oracle success to $0.90$, so the gap opens from $+0.30$ to $+0.90$ at $n = 10$. The \emph{planner-capacity} branch of the verdict tree (\S\ref{sec:capvscov}) was therefore a real contributor on the oracle arm but not on either learned arm.
...
The framework's adapter interface (\S\ref{sec:framework}) is what makes CPG cheap to compute: any \texttt{dynamics} callable can be swapped into the same planner without touching anything else, and any planner can be swapped against a fixed dynamics. \S\ref{sec:robustness} exercised this on a published world model (TD-MPC2) and on a CEM planner; the verdict for the v0.11 reading is robust to both swaps and the random-shooting MPC was bracketing the oracle low. Natural further axes that the same plumbing supports are a multi-seed pooled-$150$ extension of Table~\ref{tab:robustness} (the random-shoot baseline at this $n$ already exists in Table~\ref{tab:sweep}; the CEM rows would be the obvious next data point), a second environment (DMC Cartpole-swingup or Reacher) to test whether the verdict pattern generalises across tasks, and a perturbation-aware CPG that measures how the gap widens under sensor or dynamics noise (the perturbation library shipped with the framework can already inject these into either planner's run).
