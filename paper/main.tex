...
We have proposed the Counterfactual Planning Gap as a decision-grade metric for world-model evaluation, packaged it behind a minimal evaluation contract, and reported a worked example on DMC Acrobot-swingup. The headline number at $n = 10$ is \textsc{inconclusive}, which is the correct verdict at that sample size and the verdict the framework reports. We argue this kind of calibrated honesty -- a metric that refuses to claim more than the data supports -- is the right design target for a methodology paper that wants to sit usefully next to a fast-moving model literature.
