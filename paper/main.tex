...
\input{figures/coverage_histogram.tex}
...
\begin{table}[t]
  ...
  \begin{tabular}{llrrrl}
    ...
    \bottomrule
  \end{tabular}
  \caption{Cross-env CPG on DMC Cartpole-swingup. Three seeds, $10$ episodes per arm per seed, $n = 30$ pooled. Same four-arm structure as Table~\ref{tab:robustness}; only the env (\texttt{wmel.envs.dmc\_cartpole}) and a fresh TD-MPC2 training are different. The \textsc{model bottleneck} verdict reproduces in every cell. Numbers from \texttt{results/dmc\_cartpole/}.}
  \label{tab:crossenv}
\end{table}

Three observations are worth flagging. First, the verdict \textbf{reproduces}: \textsc{model bottleneck} in all four cells, on a task whose oracle reaches the upright pose ($0.500$--$0.900$ depending on planner) rather than the $0.30$--$0.90$ band of Acrobot. The metric travels across the easy/hard axis at fixed family. Second, the TD-MPC2 dynamics arm reaches \emph{non-zero} success on Cartpole ($0.200$ under random-shooting, $0.133$ under CEM) -- the first non-trivial learned-arm success in the paper. CPG still commits to \textsc{model bottleneck} because the gap is still positive and the AC lower bound is strictly above zero; the metric is tracking gap magnitude, not just gap presence. Third, the random-shooting planner now outperforms CEM on Cartpole's oracle ($0.900$ vs $0.500$), inverting the Acrobot pattern; the planner-capacity contributor that \S\ref{sec:robustness} surfaced on Acrobot is not a universal direction. CPG separates planner-capacity from dynamics-quality per-task.

\input{figures/cross_env_cpg.tex}

Figure~\ref{fig:cross_env_cpg} aggregates the cross-env picture: same verdict (\textsc{model bottleneck}) on both environments, materially different gap magnitudes and oracle baselines.
...
The framework's adapter interface (\S\ref{sec:framework}) is what makes CPG cheap to compute: any \texttt{dynamics} callable can be swapped into the same planner without touching anything else, any planner can be swapped against a fixed dynamics, and any perturbation can be plugged into the runner. \S\ref{sec:robustness} and \S\ref{sec:robustness_perturbation} exercised this on a published world model (TD-MPC2), on a CEM planner, on a pooled-$150$ multi-seed extension under CEM, and on an action-burst perturbation sweep; the verdict for the v0.11 reading is robust to all four swaps, the random-shooting MPC was bracketing the oracle low, and the pooled CI half-width is below $0.06$. Natural further axes that the same plumbing supports are a second environment (DMC Cartpole-swingup or Reacher) to test whether the verdict pattern generalises across tasks, an observation-noise perturbation that hurts the learned arms differentially (the v0.8 DMC wrapper would need a small extension to expose the hook), and a TD-MPC2-trained-from-multiple-seeds extension of the bottom block of Table~\ref{tab:robustness} (the present rows hold the TD-MPC2 agent fixed at the seed-$0$ checkpoint).
