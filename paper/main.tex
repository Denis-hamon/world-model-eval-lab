...
The framework's adapter interface (\S\ref{sec:framework}) is what makes CPG cheap to compute: any \texttt{dynamics} callable can be swapped into the same planner without touching anything else, and any planner can be swapped against a fixed dynamics. \S\ref{sec:robustness} exercised this on a published world model (TD-MPC2), on a CEM planner, and on a pooled-$150$ multi-seed extension under CEM; the verdict for the v0.11 reading is robust to all three swaps, the random-shooting MPC was bracketing the oracle low, and the pooled CI half-width is below $0.06$. Natural further axes that the same plumbing supports are a second environment (DMC Cartpole-swingup or Reacher) to test whether the verdict pattern generalises across tasks, a perturbation-aware CPG that measures how the gap widens under sensor or dynamics noise (the perturbation library shipped with the framework can already inject these into either planner's run), and a TD-MPC2-trained-from-multiple-seeds extension of the bottom block of Table~\ref{tab:robustness} (the present rows hold the TD-MPC2 agent fixed at the seed-$0$ checkpoint).
