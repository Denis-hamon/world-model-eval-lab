...
\begin{enumerate}
  ...
  \item \textbf{Heterogeneous verdicts across three tasks and two published models.} Exercised on three DeepMind Control Suite tasks (Acrobot-swingup, Cartpole-swingup, Reacher-easy), the gate fires four of its five branches on real data (\S\ref{sec:empirical}--\S\ref{sec:dreamerv3}): \textsc{planner bottleneck} on Acrobot (the oracle planner itself solves only $\sim\!3\%$ of random initial states), \textsc{model bottleneck} on Reacher, \textsc{learned outperforms oracle} on high-capacity Cartpole under CEM, and \textsc{inconclusive} on several moderate-$n$ near-ties. The same structure reproduces across two published model families (TD-MPC2 and the recurrent DreamerV3), and where both clear zero CPG ranks them. A fixed metric, or a point-estimate leaderboard, reports none of this structure.
...
\subsection{A second published model: DreamerV3 across three environments}
\label{sec:dreamerv3}

The cells so far use a bespoke MLP and TD-MPC2~\citep{hansen2024tdmpc2}. To test whether the verdict structure is a property of the metric rather than of one model, we add a second \emph{published} world model, and the first \emph{recurrent} one through the Markovian planner contract: DreamerV3~\citep{hafner2023dreamerv3}. The world-model weights are ported into the adapter ($\texttt{\_wm.*}$ keys), with numerical equivalence to the reference implementation asserted by a unit test; only the \texttt{dynamics} callable changes, so the comparison stays a clean counterfactual. The planner and ablation arms below \emph{re-plan a single trained checkpoint} -- no retraining.
...
\begin{table}[t]
  ...
  \begin{tabular}{lllrrl}
    \toprule
    Environment & Planner & Oracle / Learned & Raw CPG & AC 95\% CI & Verdict \\
    \midrule
    Acrobot ($n{=}30$)  & RS  & $0.033 / 0.033$ & $+0.000$ & $[-0.12, +0.12]$ & \textsc{planner bottleneck} \\
    Acrobot ($n{=}10$)  & CEM & $0.200 / 0.100$ & $+0.100$ & $[-0.24, +0.41]$ & \textsc{inconclusive} \\
    Cartpole ($n{=}10$) & RS  & $1.000 / 0.000$ & $+1.000$ & $[+0.61, +1.05]$ & \textsc{model bottleneck} \\
    Reacher ($n{=}30$)  & RS  & $1.000 / 0.667$ & $+0.333$ & $[+0.14, +0.49]$ & \textsc{model bottleneck} \\
    \bottomrule
  \end{tabular}
  \caption{DreamerV3 through CPG, task-level (varied initial state, three seeds pooled where $n{=}30$). The verdict heterogeneity reproduces a \emph{second} model family: \textsc{planner bottleneck} on Acrobot, \textsc{model bottleneck} on Cartpole and Reacher. Numbers from \texttt{results/dmc\_*/dreamerv3\_*.json}.}
  \label{tab:dreamerv3}
\end{table}

Three findings. First, \textbf{the verdict structure is model-independent}: DreamerV3 lands at the same three branches as TD-MPC2 -- \textsc{planner bottleneck} on Acrobot (the search, not the model, is the limit; both arms solve $\sim\!3\%$ of random starts) and \textsc{model bottleneck} on Cartpole and Reacher (Table~\ref{tab:dreamerv3}). The verdict tracks the (environment, planner) regime, not the identity of the learned model.

Second, \textbf{where both models clear zero, CPG ranks them -- and the recurrent flagship is not the better planner here}. On Reacher, DreamerV3 forfeits more planning success than TD-MPC2 ($\mathrm{CPG} = +0.333$ vs $+0.200$; Table~\ref{tab:reacher}); on Cartpole its learned dynamics fails outright ($\mathrm{CPG} = +1.000$, learned success $0.000$) where TD-MPC2 reaches $0.30$--$0.77$ depending on capacity (\S\ref{sec:crossenv}). A reconstruction- or FID-based leaderboard could rank a recurrent DreamerV3 above an MPC-tuned TD-MPC2; for \emph{decisions} on these tasks the ranking inverts. This is the prediction-versus-decision dissociation the metric exists to surface, now between two published models rather than between a metric and a verdict.

Third, \textbf{a recurrence-truncation ablation isolates a cost the adapter pays}. The adapter's \texttt{dynamics} callable re-encodes from the observation and truncates DreamerV3's recurrent state to a one-frame posterior on every call; DreamerV3's native mode instead imagines forward in latent space from a single encode. Running both as planning arms next to the oracle, the measured CPG difference on Acrobot is $0.0$ -- but \emph{both arms sit at the planner floor} ($\sim\!10\%$ success), so this is a null with near-zero power on \emph{decision quality}, not evidence the truncation is free. What is measurable there is \emph{compute}: the latent rollout plans $3.4\times$ faster ($943$ vs $3{,}226$\,ms per decision) by encoding once rather than per call. Quantifying the truncation's accuracy cost requires a non-floored task and is left to future work.
...
The empirical study spans three DMC control tasks (Acrobot-swingup, Cartpole-swingup, Reacher-easy), two planners (random-shooting and CEM), and three learned-dynamics families (a bespoke MLP, TD-MPC2, and the recurrent DreamerV3), each evaluated over the task's initial-state distribution with the two arms paired by initial state and three seeds pooled. The headline is not a single number but a \emph{pattern}: across the three tasks the gate commits to four of its five branches -- \textsc{planner bottleneck} (Acrobot), \textsc{model bottleneck} (Reacher, most Cartpole cells), \textsc{learned outperforms oracle} (high-capacity Cartpole under CEM), and \textsc{inconclusive} (several moderate-$n$ near-ties) -- and the pattern reproduces across both published model families, with CPG ranking the two where their intervals clear zero (\S\ref{sec:dreamerv3}). The study is deliberately scoped to simulated, fully-observed control where an oracle dynamics is available; on hardware-in-the-loop or physical environments the metric is undefined, and surrogate-oracle variants (e.g.\ a higher-fidelity model or a domain-randomised reference in the spirit of \citet{tobin2017domain}) are future work. The adapter interface (\S\ref{sec:framework}) is what makes this breadth cheap: each new cell is one more \texttt{dynamics} callable or planner constructor passed into the same \texttt{BenchmarkRunner}.
