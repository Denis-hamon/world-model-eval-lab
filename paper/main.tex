...
\begin{enumerate}
  ...
  \item We show, across three DeepMind Control Suite tasks (Acrobot-swingup, Cartpole-swingup, Reacher-easy), that the decision rule \emph{changes its verdict on the strength of the evidence}: it returns \textsc{inconclusive} at $n = 10$ (raw gap $+0.30$, AC CI crossing zero) and again, at moderate $n$, on the one studied cell where a smaller-capacity learned model matches the oracle ($0.533$ vs $0.500$, CI $[-0.28, +0.21]$); it commits to \textsc{model bottleneck} only once the interval clears zero (\S\ref{sec:empirical}--\S\ref{sec:reacher}). A gate that flips on evidence strength is the property a point-estimate leaderboard cannot reproduce. A supporting ablation dissociates prediction from decision quality: held-out MSE drops $\sim\!150\times$ across a training-set-size sweep while the verdict and CI do not move.
\end{enumerate}
